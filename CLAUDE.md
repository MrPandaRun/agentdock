...
AgentDock is a local-first multi-agent control plane supporting `Codex` and `Claude Code` providers in V1. It consists of a Tauri desktop app (Rust + React) and an Expo mobile app (React Native), with shared TypeScript contracts and a Rust core using SQLite.

## Commands

### Development
- `bun run dev` / `bun run dev:desktop` - Run Tauri desktop app (Vite dev server + Rust)
...
- `bun run --filter @agentdesktop/contracts test` - Run contract package tests
...
- Desktop app initializes DB at app data dir (`agentdock.db`) on startup

## Package Manager
- **Bun only** for JS workspace operations (root lockfile is `bun.lockb`)
- Do not add `pnpm-workspace.yaml` or `pnpm-lock.yaml`
- Rust toolchain pinned by `rust-toolchain.toml` (`stable`)
